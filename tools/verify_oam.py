...
def verify_battle_transition(sess):
    ...
    check("overworld steady (shadow OAM)", (64, 88), shadow_oam(sess))
...
def verify_dialogue_transition(sess):
    ...
    sess.step(1)
    check("dialogue steady: sprite visible behind box (shadow OAM)",
...
def verify_scene_transition(sess):
    ...
    check("overworld steady (shadow OAM)", (64, 152), shadow_oam(sess))
    ...
    sess.hold("RIGHT", 14)
    check("new map sprite position (shadow OAM)", (64, 27), shadow_oam(sess))
