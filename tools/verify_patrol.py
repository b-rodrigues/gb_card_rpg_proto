...
def main():
    ...
    # Observe steps over 800 frames (~25 steps)
    observed_steps = {}
    prev_step = pb.memory[ACTOR0_ADDR + 17]
    for _ in range(800):
        pb.tick()
        raw_step = pb.memory[ACTOR0_ADDR + 17]
        act_x = pb.memory[ACTOR0_ADDR + 4]
        act_y = pb.memory[ACTOR0_ADDR + 5]
